...
export default function EmergencyRecorderModal({ isOpen, onClose, showToast }) {
  ...
  const handleStopRecording = () => {
    ...
    showToast(`Emergency Encounter recording finalized! ${uploadedChunks} encrypted chunks secured to decentralized tamper-proof vault.`, 'success');
  };
        ...
        <div className="relative bg-slate-950 p-6 flex flex-col items-center justify-center min-h-[260px] border-b border-slate-800 overflow-hidden">
          ...
          <div className="text-center space-y-3 z-10">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 max-w-sm mx-auto mt-1">
                Footage is streamed in real-time off-device. Even if this phone is confiscated or damaged, your recording is permanently preserved in the cloud.
              </p>
            </div>
          </div>
          ...
          <div className="absolute bottom-3 left-4 right-4 flex items-center justify-between text-[11px] font-mono text-slate-400 bg-slate-900/70 backdrop-blur-sm px-3 py-1 rounded-lg border border-slate-800">
            <span className="flex items-center gap-1">
              <MapPin className="w-3 h-3 text-crimson-400" /> {geoLoc.address} ({geoLoc.lat}, {geoLoc.lng})
            </span>
            <span className="text-justice-400 flex items-center gap-1">
        ...
        <div className="p-6 space-y-4 bg-slate-900">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="p-3 bg-slate-950 rounded-xl border border-slate-800 space-y-1">
              ...
              <p className="text-[11px] text-slate-300">
                3 Trusted Contacts notified via SMS with live GPS tracking link & stream preview.
              </p>
            </div>
            ...
            <div className="p-3 bg-slate-950 rounded-xl border border-slate-800 space-y-1">
              ...
              <p className="text-[11px] text-slate-300">
                Connected to National Civil Rights Intake Network for immediate observer dispatch.
              </p>
            </div>
          </div>
          ...
          <div className="p-3 bg-amber-950/30 border border-amber-800/40 rounded-xl text-xs text-amber-300 space-y-1">
            <p className="font-bold flex items-center gap-1">
              <AlertTriangle className="w-3.5 h-3.5" /> What to Say Out Loud Right Now:
            </p>
          ...
          <div className="pt-2 flex items-center justify-between">
            ...
            {isRecording ? (
              <button
                ...
                className="px-6 py-2.5 bg-crimson-600 hover:bg-crimson-500 text-white rounded-xl text-xs font-bold shadow-glow-crimson flex items-center space-x-2 transition-all"
              >
