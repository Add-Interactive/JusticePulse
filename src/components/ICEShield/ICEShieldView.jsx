...
export default function ICEShieldView({ 
  onOpenSOSModal, 
  showToast,
  onClose,
  onOpenSettingsModal,
  ...
  const [activeSubTab, setActiveSubTab] = useState('scenarios'); // 'scenarios' | 'juvenile' | 'broadcast' | 'radar' | 'sanctuary_map' | 'family_plan' | 'hotlines'
  ...
  return (
    <div className={`fixed inset-0 z-50 bg-[#080c14] text-white flex flex-col overflow-y-auto selection:bg-crimson-600 selection:text-white select-none ${currentTheme} ${isHighContrast ? 'high-contrast-mode' : ''}`}>
      {/* Full-Screen Sticky Header Bar */}
      <header className="sticky top-0 z-40 bg-[#0c101c]/98 backdrop-blur-md border-b-2 border-crimson-800/60 px-4 sm:px-8 py-3 flex items-center justify-between shadow-2xl flex-shrink-0">
        {/* Left: Brand & Portal Title */}
        <div className="flex items-center space-x-3">
          <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-2xl bg-gradient-to-br from-crimson-500 via-pink-500 to-amber-500 p-0.5 shadow-glow-crimson flex items-center justify-center flex-shrink-0">
          ...
          <div>
            <div className="flex items-center space-x-2">
              <span className="font-display font-black text-base sm:text-lg tracking-tight text-white">
                ICE &amp; TACTICAL<span className="text-crimson-400"> SHIELD</span>
              </span>
              <span className="text-[9px] font-mono px-2 py-0.5 rounded-full bg-crimson-950 text-crimson-200 border border-crimson-600 font-bold hidden sm:inline">
                CIVIL DEFENSE COMMAND
              </span>
            </div>
            <p className="text-[10px] text-crimson-200/80 font-mono hidden md:block leading-none">
              Constitutional Rights, Digital Red Cards, Warrant OCR &amp; Youth Interrogation Lock
            </p>
          </div>
        </div>

        {/* Center: Live Ticker */}
        <div className="hidden lg:flex items-center space-x-3 text-xs font-mono bg-[#111726] px-3.5 py-1.5 rounded-2xl border border-crimson-700/60">
          <span className="text-crimson-300 font-bold">4TH &amp; 5TH AMENDMENT</span>
          <span className="text-slate-500">•</span>
          <span className="text-slate-300">10 LANGUAGES</span>
          <span className="text-slate-500">•</span>
          <span className="text-emerald-400 font-bold">PAYTON V. NY SANCTUARY</span>
        </div>
        ...
        <div className="flex items-center space-x-2 sm:space-x-3 text-xs">
          {onOpenSettingsModal && (
            <button
              ...
              className="p-2 sm:px-3 sm:py-1.5 rounded-xl bg-[#111726] hover:bg-[#1a243b] text-slate-200 hover:text-white border border-slate-700 font-mono font-bold flex items-center gap-1.5 transition-all"
          ...
          {onOpenSOSModal && (
            <button
              ...
              className="hidden sm:flex items-center space-x-1.5 px-3 py-1.5 rounded-xl bg-gradient-to-r from-crimson-600 to-pink-600 hover:from-crimson-500 text-white font-mono font-bold shadow-glow-crimson transition-all"
            >
              <Video className="w-3.5 h-3.5 animate-pulse" />
              <span>SOS Live Stream ➔</span>
            </button>
          )}
          ...
          {onClose && (
            <button
              onClick={onClose}
              className="px-3.5 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-white border border-slate-600 font-mono font-bold flex items-center gap-1.5 transition-all active:scale-95 shadow-md"
              title="Close ICE Shield and Return to Main Platform"
            >
              <ArrowLeft className="w-4 h-4" />
              <span>Exit Shield</span>
            </button>
          )}
        </div>
      </header>
